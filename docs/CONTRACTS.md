...
### 1.1 Managed-routing contract reconciliation
...
The registry and every safe projection use one fixed provider order: Mistral `1`, Google Gemini `2`, Groq `3`, OpenAI `4`, and local replay `5`. Live evaluation status is derived from a version-controlled static admission record. It is never promoted by an environment value, a runtime evaluation-file read, or a provider response. Missing, incomplete, or mismatched live admission evidence fails closed as `not_evaluated`. Local replay reports `evaluationStatus: not_applicable` and remains visibly governed by deterministic replay assurance instead of live-provider admission.

That registry and display order is distinct from live execution preference. `ANALYSIS_PROVIDER_ORDER` may contain one server-only comma-separated permutation of the four live provider IDs, each exactly once. Missing configuration uses the default Mistral, Gemini, Groq, OpenAI execution order. Empty, duplicate, incomplete, or unknown values reject the routing request before any provider call. This setting cannot alter the registry, static admission, release configuration, enablement, credentials, data-origin eligibility, public availability projection, or replay position.
...
Mistral and Gemini have exactly one allowlisted bundled-fixture binding. Groq and OpenAI may additionally accept the provider-neutral browser intent only when it carries an attested `browser_local` origin, a server-recomputed approved-redacted-input digest, the current document-set and masking revisions, and an exact canonical source map. Raw PDFs and unmasked text are excluded. A matching data-origin label without the remaining canonical checks is insufficient.
...
type SystemCardBase = {
  ...
  providerDisplayOrder: ["mistral", "google_gemini", "groq", "openai", "local_replay"];
...
Evidence and report statuses are derived, never independently asserted. A complete executed evidence record is `passed` exactly when every definition-required check appears once and passes, and `failed` exactly when every required check appears once and at least one fails. A live record is `interrupted` only for the allowlisted operational classifications `provider_quota_exhausted`, `provider_rate_limited`, `provider_timeout`, or `provider_unavailable`. It retains an ordered `providerAttempts` history with every real run ID, time, terminal status, provider provenance, transmission fact, outcome, and safe interruption classification. An explicit resume appends a new attempt; it never erases the interruption. `not_run` has no execution, no run time, null actual run ID, and only the definition's planned release and not-run markers. `DeterministicHarnessResult` is a separate zero-network mock-harness artifact used to exercise every definition in CI. It never enters `ProviderEvaluationAdmissionReport.evidence` and cannot support live admission, including for a model-output definition. A gate is `passed` exactly when all required, definition-matched admission evidence records for that gate are complete and passed; it is `failed` exactly when that complete set contains a failed record; otherwise, including when evidence is interrupted, it is `not_run`. A report is `passed` only when its complete gate set is passed, `failed` only when its complete gate set contains a failed gate, and `incomplete` otherwise. Any mismatch between evidence status, attempt history, checks, gate references, or report status rejects the artifact before digest or static handoff.
...
27. Managed live routing preserves Mistral `1`, Google Gemini `2`, Groq `3`, then OpenAI `4`. Local replay is outside this order.

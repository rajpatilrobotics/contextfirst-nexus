...
Live analysis uses a provider-neutral server boundary whose default consideration order is Mistral, Gemini, Groq, then OpenAI. The complete order may be changed only through the strict server-only `ANALYSIS_PROVIDER_ORDER` setting; invalid values fail closed without a provider call, and ordering never bypasses admission, enablement, configuration, or data eligibility. Free Mistral and unpaid Gemini remain restricted to the exact bundled synthetic fixture, so browser-uploaded packets skip them without a call. Groq `openai/gpt-oss-120b` and OpenAI may receive only explicitly approved redacted text from attested synthetic or authorized-public browser-local packets, and only after their exact evaluation, reviewed static admission, credentials, data-policy controls, and deployment approval pass. No live release is currently admitted or enabled. Bundled replay remains local and separate from live-provider routing.

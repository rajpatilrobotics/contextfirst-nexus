...
function reviewCandidate(page: Page, candidateId: string) {
  return page.locator(`article#candidate-${candidateId}`);
}
...
test.describe("TASK-025 prepared checkpoint rehearsal", () => {
  test("rehearses the constrained local prepared-checkpoint judged flow", async ({ page }) => {
    ...
    await expect(
      page.getByText(
        /Checkpoint active with fixture-reviewer provenance and no provider transmission/i,
      ),
    ).toBeVisible();
    ...
    await expect(
      page
        .getByLabel("Review workspace")
        .getByText("Prepared demo review checkpoint", { exact: true }),
    ).toBeVisible();
    ...
    await page.getByText(/^Previously reviewed items/).click();
    await expect(reviewCandidate(page, "CAND-TASK-0402")).toContainText(
      "Human accepted",
    );

    const heroCandidate = reviewCandidate(page, "CAND-TASK-0402");
    ...
    await recordReasonedAction(
      reviewCandidate(page, "CAND-CTRL-PASSPORT"),
      /Edit wording/i,
    ...
    await expect(reviewCandidate(page, "CAND-CTRL-PASSPORT")).toContainText(
      "Human edited",
    );

    await recordReasonedAction(
      reviewCandidate(page, "CAND-CTRL-CONFINEMENT"),
      /Reject suggestion/i,
      "The packet does not independently confirm physical confinement.",
    );
    await expect(reviewCandidate(page, "CAND-CTRL-CONFINEMENT")).toContainText(
      "Rejected",
    );

    await recordReasonedAction(
      reviewCandidate(page, "CAND-PROV-TASKLOG"),
      /Reject suggestion/i,
      "Unresolved provenance cannot support an export finding.",
    );
    await expect(reviewCandidate(page, "CAND-PROV-TASKLOG")).toContainText(
      "Rejected",
    );
    ...
    await page.goto("/case/demo/review");
    await recordReasonedAction(
      reviewCandidate(page, "CAND-SENDER-0402"),
      /Reject suggestion/i,
      "Assignment and allegation records do not establish sender identity.",
    );
    await reviewCandidate(page, "CAND-URG-INTERPRETER")
      .getByRole("button", { name: /Confirm as unknown/i })
      .click();
    await expect(reviewCandidate(page, "CAND-SENDER-0402")).toContainText(
      "Rejected",
    );
    await expect(reviewCandidate(page, "CAND-URG-INTERPRETER")).toContainText(
      "Human accepted",
    );

    await reviewCandidate(page, "CAND-TASK-0402")
      .getByRole("button", { name: /Withdraw evidence/i })
      .click();
    ...
    await reviewCandidate(page, "NEXUS-COMPELLED-TASKS")
      .getByRole("button", { name: /Accept suggestion/i })
      .click();
    await expect(
      reviewCandidate(page, "NEXUS-COMPELLED-TASKS"),
    ).toContainText("Human accepted");

    await recordReasonedAction(
      reviewCandidate(page, "NEXUS-OFFENCE-TIMING"),

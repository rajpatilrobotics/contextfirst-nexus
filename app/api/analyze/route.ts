...
import { AnalyzeAvailabilityResponseSchema, AnalyzeResponseSchema } from "../../../lib/contracts";
...
export async function POST(request: NextRequest) {
  ...
  const result: any = await analyze(body);
  return json(result, result.outcome === "rejected_before_run" ? 400 : 200);
}

function rejected(error: ReturnType<typeof makePreflightError>) {

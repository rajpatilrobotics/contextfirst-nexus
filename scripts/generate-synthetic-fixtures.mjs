...
function buildGuidancePack() {
  const cards = [
    ...
    ["INT-004", "UNODC", "Issue paper on non-punishment of victims of trafficking", "international_guidance"],
    ["HR-002", "UN Human Rights", "Human rights and trafficking protections", "treaty"],
    ["IND-001", "ILO", "Forced labour indicators", "operational_indicator"],
    ["FC-002", "OSCE", "Forced criminality and non-punishment guidance", "report"],
    ["SEC-001", "OWASP", "Prompt injection and untrusted content guidance", "security_guidance"],
  ].map(([id, issuer, title, materialType]) => ({
    id: `GUIDANCE-${id}`,
    sourceRegisterId: id,
    issuer,
    title,
    materialType,
    publicationOrVersionDate: "Registered source version",
    sourceVersion: "registered",
    jurisdictionOrScope: "International or security guidance; local legal verification required",
    exactReviewedPassage: "Reviewed public guidance passage summarized for practitioner question-framing only.",
    locator: `docs/SOURCE_REGISTER.md ${id}`,
    sourceUrl: "https://example.org/",
    lastVerified: LAST_VERIFIED,
    verificationStatus: "current_for_scope",
    localLegalVerificationRequired: true,
    allowedUse: "May frame practitioner review questions and safety checks; never proves a case fact.",
    limitation: "Does not establish domestic law, victim status, guilt, credibility, legal eligibility, or endorsement.",
  }));
  const projection = { schemaVersion: VERSION, cards };
  return {
    schemaVersion: VERSION,
